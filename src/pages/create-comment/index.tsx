...
import { chooseImage, getCurrentInstance, showToast } from '@tarojs/taro';
import { View, Textarea, BaseEventOrig } from '@tarojs/components'
import { Uploader, Button, Toast } from "@taroify/core"

import { TextareaProps } from '@tarojs/components/types/Textarea';
import { CommentType, createComments, createInstantaneous, getCurrentUser, isAdmin, uploadImage } from '../../api';
...
function IndexPage(){
  ...
  const handlePublishInstantance = useCallback(function handlePublishInstantance() {
    ...
    const infoId = getCurrentInstance()?.router?.params?.infoId;
    if (!infoId) {
      return Toast.open({ type: 'fail', message: '非法活动 ID' });;
    }
    const userId = getCurrentUser().userId;
    if (!userId) {
      return Toast.open({ type: 'fail', message: '请先登录' });
    }
    ...
    //   content: context,
    //   type: CommentType.COMMNET,
    //   // informationType: isCreator ? InformationTypeEnum.A
    //   urls: urls ?? [],
    // });
    isAdmin(infoId, userId).then(isCreator => {
      createComments({
        userId,
        informationId: infoId,
        content: context,
        type: isCreator ? CommentType.PROGRESS : CommentType.COMMNET,
        // informationType: isCreator ? InformationTypeEnum.A
        urls: urls ?? [],
      }).then(() => {
        showToast({ title: '发表评论成功' });
      });
    })
  }, [files, context]);
